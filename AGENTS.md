...
This file governs unattended/scheduled agent runs on this repo, whichever
agent is driving: Claude Code (the `lmdb-autonomous-dev` scheduled routine),
Antigravity, or anything else. A human-driven session can ignore this file,
but should still respect the conventions below.

**One contract, several tools.** `AGENTS.md` is the only real copy.
`CLAUDE.md` and `.claude/CLAUDE.md` are symlinks to it, so Claude Code and
Antigravity read byte-identical rules and the contract cannot drift between
them. Edit `AGENTS.md`; never replace a symlink with a copy.

Where each tool looks for the rest of its configuration:

| | Claude Code | Antigravity |
|---|---|---|
| Contract | `CLAUDE.md` → `AGENTS.md` | `AGENTS.md` |
| Skills | `.claude/skills/` | `.agents/skills/` → `.claude/skills/` |
| Hooks | `.claude/settings.json` | `.agents/hooks.json` |
| MCP servers | `.mcp.json` | `.agents/mcp_config.json` |

The skills directory is shared by symlink for the same reason as the
...
The `.claude/` path below is deliberate and shared by every agent,
Antigravity included. Two tools using two different lock paths would not see
each other's lock and would happily race — the path is a coordination point,
not a Claude Code detail.
...
- GitHub Issues on `liviuionesi/lmdb.dev` are the backlog.
...
1. Run `gh issue list --repo liviuionesi/lmdb.dev --state open`.
...
4. Code review: a second, independent pass — a subagent in Claude Code, a
   separate agent run in Antigravity, or the appropriate devkit review
   skill for the stack. For large diffs, or
...
- Never close an issue with an unchecked acceptance-criteria box. Never
  ...
  service directly instead of through the gateway). Ticking a box you
  haven't actually verified is worse than leaving it open — if the
  evidence isn't there, the issue isn't closing. If a later decision
  superseded the work, rescope the criterion in one line and name the ADR
  or issue that replaced it.
- **Keep `## Notes` short.** It holds decisions, rescopes, known gaps and
  follow-up issue numbers — the things someone needs months later. It is
  ...
  what the environment couldn't run. If it's longer than a short
  paragraph or two, it's a transcript, not a note.
