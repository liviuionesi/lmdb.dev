package com.filmpire.config.observability;
 ...
 *
 * <p>Uses {@link TestRestTemplate} against a RANDOM_PORT context rather than MockMvc: this module
 * does not carry the webmvc test-autoconfigure artifact, and the pattern here matches the module's
...
class PrometheusEndpointTest {
   ...
   * because a test profile may rename the application — the contract is "tagged with THIS service's
   * name", not one literal.
   */
   ...
   * jvm_memory_used_bytes} means nothing without knowing which service emitted it. The common
   * {@code management.metrics.tags.application} tag is what lets a shared Grafana dashboard split
   * by service.
   */
  @Test
