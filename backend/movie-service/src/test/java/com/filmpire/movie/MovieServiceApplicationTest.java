...
class MovieServiceApplicationTest extends AbstractMongoIntegrationTest {
  ...
  /**
   * Receives the fully started context via constructor injection so it can be inspected by the
   * smoke tests below.
   */
  ...
  /**
   * Switches off caching, Eureka and Cloud Config (the Mongo URI itself comes from {@link
   * AbstractMongoIntegrationTest}). Without these overrides the context would try to reach
   * infrastructure that does not exist in CI, and the smoke test would fail for the wrong reason.
   */
  ...
  /**
   * Any broken bean definition (missing dependency, bad property, failed auto-configuration) aborts
   * context startup before this method runs, so merely reaching the assertion proves the whole
   * application wiring is valid against a real MongoDB.
   */
  @Test
  ...
  /**
   * Guards against beans silently dropping out of component scanning (e.g. a package move or class
   * rename): the context can still start "successfully" without them, so the core service,
   * repository and both controllers are asserted by bean name.
   */
  @Test
