 ...
 *
 * <p>Guards a failure mode this repo has already hit: {@code application.yml} listed {@code
 * prometheus} under the actuator exposure list, which looks complete on inspection, but no module
 * carried {@code micrometer-registry-prometheus}. Without a registry on the classpath Boot never
 * creates the endpoint, so the scrape URL 404s and Prometheus silently records nothing while the
 * config appears correct. Config alone is therefore not evidence — only a real request is.
 *
 * <p>Boots the full context against the module's shared Testcontainers MongoDB ({@link
...
class PrometheusEndpointTest extends AbstractMongoIntegrationTest {
   ...
   * common {@code management.metrics.tags.application} is what makes a shared Grafana dashboard
   * able to split by service, so it is worth pinning rather than leaving to config review.
   */
  @Test
