...
/**
 * Web-layer tests for the native {@link MovieController} ({@code @WebMvcTest} slice with a mocked
 * {@link MovieService}).
 *
 * <p>Scope is the HTTP contract only: routing, path/query-param binding, and the JSON serialization
 * of each response — the service is stubbed, so these tests isolate "does the controller expose the
...
class MovieControllerTest {
  ...
  /**
   * Details endpoint must bind the {@code {id}} path variable and serialize the full DTO including
   * nested genres — the movie details page's primary fetch.
   *
  ...
  void getMovieById_ShouldReturnMovieDetails() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$._links.recommendations.href",
                containsString("/api/v1/movies/550/recommendations")));
  }

  /**
   * Discover must serialize the PageResponse including pagination metadata (content, totalElements,
   * totalPages, pageNumber) — the fields the UI needs to drive its pager.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  /**
   * The {@code genreId} query param must bind and reach the service (verified via the stubbed
   * argument), so genre browsing filters correctly.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  /**
   * The {@code year} and {@code minRating} params must bind together and be forwarded to the
   * service — covers multi-filter binding, a common miss when params share an endpoint.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  /**
   * Search must bind the {@code query} param and return a paged result — the endpoint behind the
   * app's search bar.
   *
   * @throws Exception if the mock request fails
   */
  ...
  /**
   * Trending must bind an explicit {@code timeWindow} param and forward it — paired with the
   * default-window test below to cover both cases.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  /**
   * With no {@code timeWindow} supplied the controller must default to "week" (the stub only
   * matches "week"), so the endpoint is usable without the optional param.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  void getTrendingMovies_WithDefaultTimeWindow_ShouldUseWeek() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/v1/movies/trending").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk());
  }
  ...
  /**
   * Top-rated must route at its hyphenated path {@code /top-rated} — the URL spelling matters, so
   * this pins it distinctly from popular.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  /**
   * The videos sub-resource must serialize each video with the fields the UI embeds a trailer from
   * (type + site), asserted on the first element.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  void getMovieVideos_ShouldReturnVideos() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/v1/movies/{id}/videos", movieId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  /**
   * The credits sub-resource must serialize cast and crew as separate arrays with top billing first
   * — the shape the cast section renders from.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  void getMovieCredits_ShouldReturnCredits() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/movies/{id}/credits", movieId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  /**
   * The similar sub-resource must bind {@code {id}} plus paging and route correctly — a
   * details-page rail.
   *
   * @throws Exception if the mock request fails
   */
  ...
  void getSimilarMovies_ShouldReturnSimilarMovies() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/movies/{id}/similar", movieId)
                .param("page", "1")
                .param("size", "20")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk());
  ...
  /**
   * The recommendations sub-resource must bind {@code {id}} plus paging and route at its own path —
   * the sibling of similar movies.
   *
   * @throws Exception if the mock request fails
   */
  @Test
  ...
  void getRecommendedMovies_ShouldReturnRecommendations() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/movies/{id}/recommendations", movieId)
                .param("page", "1")
                .param("size", "20")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk());
  ...
  /**
   * Builds a two-movie page (100 total, page 0, size 20) returned by all the list-endpoint stubs.
   *
  ...
  /**
   * Builds a credits DTO (two cast in billing order, one director) for the credits test.
   *
