...
/**
 * Tests for the native {@link MovieService}'s hybrid-caching behavior (MongoDB long-term + TMDB
 * fallback; Redis is applied by the {@code @Cacheable} layer, not exercised here).
 *
 * <p>All collaborators are mocked, so these tests assert the CALL PATTERN rather than data: how
 * many times MongoDB is queried, whether TMDB is called, and when a save happens. That's the real
 * contract — the point of the cache is to minimize TMDB calls, so the verify(...) counts ARE the
 * behavior under test. The double-check-inside-lock pattern (the service's single-flight guard) is
 * why some tests stub {@code findByTmdbId} to return different values across successive calls.
 */
...
class MovieServiceCacheTest {
  ...
  /** Injects the @Value-bound TMDB key that isn't populated without a Spring context. */
  @BeforeEach
  void setUp() {
  ...
  /**
   * The cache-hit fast path: when the movie is already in MongoDB, TMDB must NOT be called at all
   * ({@code verifyNoInteractions(tmdbClient)}) — this is the primary way the service stays under
   * TMDB's rate limit.
   */
  @Test
  ...
  /**
   * The cache-miss path: with the movie absent, the service must call TMDB once and persist the
   * result. The two findByTmdbId calls reflect the double-check around the single-flight lock
   * (check, lock, re-check).
   */
  @Test
  ...
  /**
   * Repeated reads of a cached movie must each hit MongoDB but never TMDB — ten calls produce ten
   * DB lookups and zero API calls, confirming the cache absorbs read volume without amplifying it
   * onto the rate-limited upstream. (Sequential here; true concurrency is out of scope for a mock
   * test.)
   */
  @Test
  ...
  /**
   * The save-then-serve sequence: after a first call fetches and persists, a second call must be
   * served from MongoDB — so across both calls TMDB is hit exactly once and save happens once. The
   * staged findByTmdbId stubs (empty, empty, present) model check→lock-recheck→subsequent-hit.
   */
  @Test
