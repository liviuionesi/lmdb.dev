...
/**
 * Coarse performance/smoke tests for {@link MovieRepository} against real MongoDB (Testcontainers,
 * {@code @DataMongoTest}).
 *
 * <p>These are NOT precise benchmarks — the wall-clock thresholds are deliberately generous
 * ceilings (seconds/hundreds of ms) that seed hundreds to a thousand documents and assert the
 * operation completes within a sane bound. Their real value is catching accidental O(n) blowups or
 * a query that stops using its index (e.g. a full-collection scan) at moderate data volume, not
...
class MovieRepositoryPerformanceTest extends AbstractMongoIntegrationTest {
  ...
  /**
   * Bulk-inserting 100 movies via saveAll must finish well within 5s — guards against a regression
   * where saveAll degrades to per-document round trips.
   */
  @Test
  ...
  void bulkInsert_ShouldBeEfficient() {
    ...
    for (int i = 1; i <= 100; i++) {
      movies.add(
          createTestMovie(
              (long) i, "Movie " + i, 7.0 + (i % 3), LocalDate.of(2020, 1, 1).plusDays(i)));
    }
  ...
  /**
   * A findByTmdbId lookup among 1000 documents must return in &lt;100ms — effectively asserting the
   * query uses the tmdbId index rather than scanning the collection (the difference a missing index
   * would make is far larger than this ceiling).
   */
  @Test
  ...
  void indexedQuery_ShouldBeEfficient() {
    ...
    for (int i = 1; i <= 1000; i++) {
      movies.add(
          createTestMovie(
              (long) i, "Movie " + i, 7.0 + (i % 3), LocalDate.of(2020, 1, 1).plusDays(i % 365)));
    }
  ...
  /**
   * Fetching a first and a deep (6th) page from 500 sorted documents must finish within 1s and
   * report the correct total — confirms deep pagination doesn't degrade badly and that
   * totalElements is accurate for the pager.
   */
  @Test
  ...
  /**
   * Querying an embedded-array field (genre id) across 200 mixed-genre movies must finish within
   * 500ms and return matches — the array-membership query is the most likely to scan, so this
   * bounds it at moderate volume.
   */
  @Test
  ...
  /**
   * A date-range query over a full year of documents must finish within 500ms and return the
   * in-range subset — bounds the range scan used by year-based browsing.
   */
  @Test
  ...
  /**
   * Case-insensitive title search across 100 documents (with two "Dark Knight" titles among noise)
   * must return exactly the two matches within 500ms — bounds the substring/regex title query and
   * confirms it stays selective amid noise.
   */
  @Test
  ...
  void textSearch_ShouldBeEfficient() {
    ...
    // Add noise data
    for (int i = 5; i <= 100; i++) {
      movies.add(createTestMovie((long) i, "Random Movie " + i, 7.0, LocalDate.of(2020, 1, 1)));
    }
    movieRepository.saveAll(movies);
    ...
    long startTime = System.currentTimeMillis();
    Page<Movie> darkKnightMovies =
        movieRepository.findByTitleContainingIgnoreCase("dark knight", pageable);
    long endTime = System.currentTimeMillis();
  ...
  /**
   * Three existence checks against 1000 documents must together finish in &lt;100ms — the
   * existsByTmdbId probe is on the hot read-through path, so it must be index-backed and cheap even
   * at volume.
   */
  @Test
  ...
  /**
   * Builds a persistable movie with the given identifying fields; popularity is derived from the id
   * so seeded datasets have a stable sort order.
   *
