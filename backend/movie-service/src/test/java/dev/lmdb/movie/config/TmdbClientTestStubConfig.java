...
/**
 * Test-only configuration that overrides the real TMDB Http client with an in-memory stub. This
 * prevents integration tests from calling the real TMDB API while still exercising the full
 * controller -> service -> caching -> MongoDB stack.
 *
 * <p>Every stub answer is deterministic and derived from the input arguments (movie id, page), so
 * integration tests can predict exact field values without network access or WireMock. Import it
 * into a test with {@code @Import(TmdbClientTestStubConfig.class)}; {@code @Primary} makes it win
 * over the auto-configured HTTP-interface proxy.
 */
@TestConfiguration
public class TmdbClientTestStubConfig {

  /**
   * Supplies the stubbed {@link TmdbClient} as the primary bean so it transparently replaces the
   * real declarative HTTP client everywhere it is injected. Implemented as an anonymous class
   * rather than Mockito so all methods have real, consistent behavior by default.
   */
  @Bean
  @Primary
  public TmdbClient tmdbClientStub() {
    return new TmdbClient() {
      /**
       * Returns a fully populated details record whose title, paths, imdb id and homepage embed the
       * requested movie id, letting callers assert that the id they asked for is the one that
       * flowed through the service and persistence layers.
       */
      @Override
      ...
      @Override
      public TmdbMovieListResponse getTrendingMovies(
          String timeWindow, String apiKey, Integer page) {
        return buildStubMovieList(page);
      }
      ...
      /**
       * Returns a single official YouTube trailer whose id and key embed the movie id, so video
       * lookups can be traced back to the requested movie in assertions.
       */
      @Override
      public TmdbVideosResponse getMovieVideos(Long movieId, String apiKey) {
        TmdbVideosResponse.TmdbVideo video =
      ...
      /**
       * Returns one cast member and one crew member (a director), the minimum shape needed to
       * verify that both credit collections survive the mapping pipeline.
       */
      @Override
      public TmdbCreditsResponse getMovieCredits(Long movieId, String apiKey) {
        TmdbCreditsResponse.TmdbCast cast =
            new TmdbCreditsResponse.TmdbCast(
                1L, "Stub Actor", "Stub Character", "/stub/actor.jpg", 0, 100L);

        TmdbCreditsResponse.TmdbCrew crew =
            new TmdbCreditsResponse.TmdbCrew(
                2L, "Stub Director", "Director", "Directing", "/stub/director.jpg", "crew-1");
