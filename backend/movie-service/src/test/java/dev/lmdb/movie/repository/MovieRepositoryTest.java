package dev.lmdb.movie.repository;
...
/**
 * Integration tests for {@link MovieRepository} against real MongoDB (Testcontainers,
 * {@code @DataMongoTest} slice — only the data layer loads).
 *
 * <p>Spring Data derives these finder queries from method names at runtime, so a mistyped property
 * or wrong keyword produces a WRONG query, not a compile error. Running them against a real Mongo
 * is the only way to prove each derived query (by-title-ignore-case, by-embedded-genre-id, date
 * range, min-rating, and the three sort orders) actually returns what its name promises.
 */
@DataMongoTest
...
class MovieRepositoryTest extends AbstractMongoIntegrationTest {
  ...
  /**
   * Baseline persistence: a saved movie gets an id and is retrievable by its TMDB id — the core
   * lookup the read-through cache depends on.
   */
  @Test
  ...
  /**
   * Title search must be substring AND case-insensitive: querying "fight" must match "Fight Club"
   * and exclude the others — the behavior a search box needs, and easy to get wrong in a derived
   * query.
   */
  @Test
  ...
  /**
   * Genre filtering queries INTO an embedded array: a movie matches if the genre id appears in its
   * nested genres list. The fixture (one action-only, one drama-only, one both) proves the query
   * matches on membership, not on a scalar field.
   */
  @Test
  ...
  /**
   * The date-range query must be inclusive-between: only Fight Club (1999) falls in 1995–2000,
   * excluding the 1994 and 2008 films — pins the boundary semantics that "year filter" browsing
   * relies on.
   */
  @Test
  ...
  /**
   * The min-rating query must be greater-than-OR-EQUAL: with threshold 8.5, the 8.8 and 9.0 films
   * match and the 8.4 and 6.5 films don't — confirms the inclusive boundary of the "minimum rating"
   * filter.
   */
  @Test
  ...
  /**
   * Popularity sort must be DESC: the fixture's popularities (450/320/890) must come back Dark
   * Knight → Fight Club → Forrest Gump. The distinct values make a wrong sort direction obvious.
   */
  @Test
  ...
  /**
   * Vote-average sort must be DESC (9.0 → 8.8 → 8.4) — the "top rated" feed's ordering, verified
   * independently of the popularity sort since they key on different fields.
   */
  @Test
  ...
  /**
   * Release-date sort must be DESC (2008 → 1999 → 1994) — the "newest first" ordering, the
   * date-typed counterpart to the numeric sorts above.
   */
  @Test
  ...
  /**
   * The existence check must return true for a stored id and false otherwise — the cheap "is this
   * already cached?" probe the service uses to decide whether to call TMDB, so both branches are
   * asserted.
   */
  @Test
  ...
  /**
   * A lookup for an unknown id must return an empty Optional (not null), which is exactly the
   * "miss" signal the read-through relies on to fall through to TMDB.
   */
  @Test
  ...
  /**
   * Builds a persistable movie with the given identifying fields and sensible defaults for the
   * rest.
   *
  ...
  private Movie createTestMovie(
      Long tmdbId, String title, Double voteAverage, LocalDate releaseDate) {
