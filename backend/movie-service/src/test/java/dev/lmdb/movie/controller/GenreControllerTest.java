...
/**
 * Web-layer tests for {@link GenreController} ({@code @WebMvcTest}: only the MVC slice loads, the
 * service is a {@code @MockitoBean}).
 *
...
class GenreControllerTest {
  ...
  /**
   * The endpoint must serialize the genre list to JSON in order (id + name per element), since the
   * sidebar renders genres exactly as returned.
   *
  ...
  /**
   * With no genres the endpoint must return 200 and an empty JSON array (not 404 or null), so the
   * client can render an empty sidebar without special error handling.
   *
   * @throws Exception if the mock request fails
   */
  @Test
