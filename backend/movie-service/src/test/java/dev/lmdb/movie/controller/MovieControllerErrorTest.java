...
/**
 * Error-handling tests for the movie web layer ({@code @WebMvcTest} slice with a mocked service).
 *
 * <p>Two kinds of failure are covered: request-shape errors the controller rejects itself (missing
 * required {@code query} param, non-numeric id → 400), and the service's declared exception
 * contract (ResourceNotFound / ServiceUnavailable / RuntimeException). Note the exception-contract
 * tests assert propagation from the stubbed service rather than the mapped HTTP status — they pin
...
class MovieControllerErrorTest {
  ...
  /**
   * A missing movie must surface as ResourceNotFoundException from the service — the signal the
   * controller/handler turns into a 404. Pins the exception type so a change to a silent-null
  ...
  /**
   * A TMDB outage must surface as ServiceUnavailableException (the signal for a 503), distinct from
   * not-found — the two map to different statuses, so the exception type must be preserved.
   */
  @Test
  ...
  /**
   * An unexpected RuntimeException must propagate (to become a generic 500) rather than be
   * swallowed — pins that unclassified failures still surface as errors instead of being masked.
   */
  @Test
  ...
  /**
   * The {@code query} param is required, so search without it must be rejected with 400 BEFORE
   * reaching the service — a real controller-level validation check (not a stubbed exception),
   * guarding against empty searches hitting TMDB.
   *
  ...
  /**
   * The {@code id} path variable is typed Long, so a non-numeric value must be rejected with 400 by
   * type conversion before the service is invoked — confirms malformed URLs fail fast rather than
   * throwing deeper.
   *
   * @throws Exception if the mock request fails
   */
  ...
  /**
   * Search must propagate a service outage as ServiceUnavailableException (→ 503) rather than
   * returning empty results, so a TMDB failure isn't misread by the client as "no matches".
   */
  @Test
  ...
  /**
   * Discover must likewise propagate an outage as ServiceUnavailableException rather than an empty
   * page — same "don't disguise failure as no results" guarantee on the browse path.
   */
  @Test
  ...
  /**
   * Missing videos for a movie must surface as ResourceNotFoundException (→ 404), so the details
   * page can distinguish "no videos" from a broken request.
   */
  @Test
  ...
  /**
   * Missing credits must likewise surface as ResourceNotFoundException (→ 404) — the credits
   * counterpart of the videos case.
   */
  @Test
