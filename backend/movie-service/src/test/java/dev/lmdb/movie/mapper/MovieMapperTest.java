...
/**
 * Tests for {@link MovieMapper}, the MapStruct mapper between persistence entities and native
 * {@code /api/v1} DTOs.
 *
 * <p>MapStruct generates the implementation at compile time by matching field names, so a renamed
 * field or type change silently drops from the mapping with no compile error. These tests exercise
 * the generated implementation directly (via the Mappers factory) and assert every field survives
 * each conversion — that field-by-field coverage is the only thing that catches a silently unmapped
 * property.
 */
...
class MovieMapperTest {
  ...
  /**
   * The full entity→DTO mapping must carry ALL fields, including nested genres and language lists —
   * this is the detail-endpoint conversion, so a dropped field means missing data on the movie
   * details page.
   */
  @Test
  ...
  void movieToDto_ShouldMapCorrectly() {
    ...
    Movie movie =
        Movie.builder()
            ...
            .revenue(100853753L)
            .spokenLanguages(
                List.of(SpokenLanguage.builder().iso6391("en").name("English").build()))
            .originalLanguage("en")
            .popularity(450.5)
            .adult(false)
            .imdbId("tt0137523")
  ...
  /**
   * The lighter list DTO (used in grid/search results) must map its subset of fields correctly — a
   * separate mapping from the full DTO, so it needs its own coverage to catch a list-specific
   * omission.
   */
  @Test
  ...
  /**
   * The Genre→GenreDto mapping (id + name) must be exact — it's reused inside the movie mapping, so
   * a bug here would corrupt genres everywhere.
   */
  @Test
  @DisplayName("Should map Genre to GenreDto correctly")
  void genreToDto_ShouldMapCorrectly() {
    // Arrange
    Genre genre = Genre.builder().id(28L).name("Action").build();
  ...
  /**
   * The list-mapping variant must preserve both element content AND order, since genres are
   * displayed in the order TMDB returns them.
   */
  @Test
  ...
  /**
   * Video mapping must carry the fields the UI needs to embed a trailer (site + key build the
   * YouTube URL; official/type drive selection), so all of them are asserted.
   */
  @Test
  ...
  /**
   * Cast mapping must preserve the billing {@code order} field (0 = top billing) alongside
   * character/profile, because the UI sorts and displays cast by that order.
   */
  @Test
  ...
  /**
   * Crew mapping must carry job + department (used to surface the director and group crew), a
   * distinct shape from cast that needs its own check.
   */
  @Test
  ...
  /**
   * Mapping a sparse entity must yield a DTO with nulls preserved (not an NPE and not defaulted
   * values) — MapStruct must pass TMDB's omitted fields through faithfully.
   */
  @Test
  ...
  /**
   * The bulk movies→list-DTO mapping must preserve element content and order, since this is the
   * conversion applied to every paged catalog/search response.
   */
  @Test
  ...
  void moviesListToDto_ShouldMapCorrectly() {
    ...
    List<Movie> movies =
        Arrays.asList(
            Movie.builder().tmdbId(550L).title("Fight Club").voteAverage(8.4).build(),
            Movie.builder().tmdbId(13L).title("Forrest Gump").voteAverage(8.8).build());
