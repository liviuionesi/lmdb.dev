...
class ErrorResponseTest {
  ...
  /**
   * Verify that the of() factory method correctly populates status, errorCode, message, path, and
   * creates a default non-null timestamp with no field errors.
   */
  @Test
  @DisplayName("of() builds a response with a non-null timestamp and no field errors")
  void of_shouldBuildSimpleResponse() {
    ErrorResponse response =
        ErrorResponse.of(404, "NOT_FOUND", "Movie not found", "/api/v1/movies/999");
  ...
  /**
   * Verify that the withValidationErrors() factory method correctly copies the validation field
   * error map into the error response.
   */
  @Test
  ...
  /**
   * The no-args constructor plus the {@code @Builder.Default} timestamp must still populate a
   * timestamp — callers may bypass the {@code of()} factory and build directly.
   */
  @Test
