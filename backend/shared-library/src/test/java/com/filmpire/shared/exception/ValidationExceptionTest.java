...
class ValidationExceptionTest {
    ...
    void messageAndFieldErrorsConstructor_shouldKeepSuppliedMap() {
        ValidationException ex = new ValidationException("invalid request", Map.of("title", "must not be blank"));
    ...
    /**
     * A null map must not leak as a null getFieldErrors() — callers rely on
     * an always-non-null map to iterate without a null check.
     */
    @Test
