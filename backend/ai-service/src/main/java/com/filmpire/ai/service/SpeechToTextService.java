...
/**
 * Offline speech-to-text, replacing the third-party Alan AI voice SDK (#68)
 * whose hosted project stopped accepting connections. Runs entirely against
 * a local <a href="https://alphacephei.com/vosk/">Vosk</a> model — no audio
 * ever leaves this service, and there is no cloud STT provider or API key
 * (ADR-004's $0 budget).
 *
 * <p>The Vosk {@link Model} is expensive to load (reads the model directory
 * from disk) and immutable once loaded, so it's loaded once, lazily, on the
 * first transcription request rather than at startup — a missing/not-yet-
 * downloaded model must not prevent ai-service's other features (chat,
 * recommendations, semantic search) from starting. A {@link Recognizer} is
 * NOT thread-safe, so a fresh one is created per request from the shared
 * {@link Model}.</p>
 */
@Service
@Slf4j
public class SpeechToTextService implements DisposableBean {
    ...
    public SpeechToTextService(
        @Value("${speech-to-text.vosk.model-path}") String modelPath,
     ...
     *
     * @param audioFile a WAV (PCM) upload; other sample rates/channel counts
     *                  are resampled to what Vosk expects
    ...
    /**
     * Loads the Vosk model on first use (double-checked locking — cheap after
     * the first call, and only one thread pays the disk-read cost).
     *
        ...
        synchronized (this) {
            if (model == null) {
                ...
                try {
                    ...
                    throw new ServiceUnavailableException("speech-to-text",
                        "model not found at '" + modelPath + "' — run infrastructure/scripts/download-vosk-model.sh first", e);
                }
            }
            return model;
        }
    ...
    /**
     * Reads an uploaded audio file and resamples it to the mono 16kHz PCM16
     * format Vosk requires, regardless of what format the browser sent.
     *
    ...
    private byte[] toPcm16Mono16kHz(MultipartFile audioFile) {
        ...
        try (AudioInputStream sourceStream = AudioSystem.getAudioInputStream(
                new BufferedInputStream(audioFile.getInputStream()))) {
            try (AudioInputStream pcmStream = AudioSystem.getAudioInputStream(TARGET_FORMAT, sourceStream)) {
                return pcmStream.readAllBytes();
            }
        } catch (UnsupportedAudioFileException | IllegalArgumentException e) {
            throw new ValidationException("audio",
                "Unsupported audio format — expected a WAV (PCM) file: " + e.getMessage());
        } catch (IOException e) {
            throw new ValidationException("audio", "Could not read the uploaded audio file: " + e.getMessage());
        }
    }
    ...
    /**
     * Frees the native Vosk model on shutdown — it holds an off-heap handle
     * that the JVM's GC doesn't know about.
     */
    @Override
    public void destroy() {
        if (model != null) {
