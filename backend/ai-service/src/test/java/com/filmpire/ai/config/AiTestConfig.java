 ...
 *
 * <ul>
 *   <li>{@link ChatModel}/{@link EmbeddingModel} become Mockito mocks — no Ollama server is
 *       reachable in CI, and stubbing at this level (rather than mocking {@code ChatClient} itself)
 *       exercises the real {@code ChatClient} fluent API and Spring AI's structured-output
 *       conversion, same as production. Each test configures the mocks' behaviour itself.
 *   <li>{@link RestClient.Builder} becomes a plain (non-{@code @LoadBalanced}) builder — {@link
 *       com.filmpire.ai.config.RestClientConfig}'s builder intercepts EVERY request host as a
 *       service-id to resolve via discovery, not just {@code lb://} URIs, so it can't be pointed at
 *       WireMock's plain {@code http://localhost:<port>} the way the real {@code
 *       lb://movie-service} is resolved via Eureka.
 * </ul>
 */
@TestConfiguration
