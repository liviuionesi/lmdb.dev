 ...
 *
 * <p>Serializable because the service layer caches it in Redis, which is configured for JDK
 * serialization in this project — a cached type without {@code Serializable} fails at runtime, not
 * compile time.
 *
 * @param id TMDB person id the images belong to
 * @param profiles every profile image TMDB holds for that person
 */
public record TmdbPersonImagesResponse(Long id, List<TmdbProfileImage> profiles)
    implements Serializable {

  /**
   * A single profile image: a TMDB CDN path plus its metadata. The bytes are never fetched or
   * stored (ARCHITECTURE.md §3.8).
   *
