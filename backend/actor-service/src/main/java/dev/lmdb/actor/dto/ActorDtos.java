package dev.lmdb.actor.dto;

import java.time.LocalDate;
import java.util.List;

/**
 * Immutable response records for the native {@code /api/v1/actors} API (records-only rule,
...
public final class ActorDtos {
   ...
   * @param popularity TMDB popularity score
   */
  public record ActorSummaryDto(Long tmdbId, String name, String profilePath, Double popularity) {}
  ...
  public record ActorSearchResponse(
      int page, int totalPages, long totalResults, List<ActorSummaryDto> results) {}

  /**
   * One profile image of an actor — a TMDB CDN reference plus metadata. The bytes are never stored
   * here (ARCHITECTURE.md §3.8); the client builds the URL from {@code filePath}.
   *
  ...
  /**
   * A page of a filmography. The native API paginates in-memory over TMDB's full credit list:
   * TMDB's own {@code movie_credits} returns every credit unpaginated, so slicing happens here
  ...
  public record FilmographyPageDto(
      int page, int totalPages, int totalItems, List<FilmographyEntryDto> results) {}
}
