 ...
 *
 * <p>The TMDB API key is injected as a default query parameter on every
 * outbound request via a {@link org.springframework.http.client.ClientHttpRequestInterceptor},
 * so business-layer code never handles or even sees the key — it is a pure
 * cross-cutting concern of the HTTP transport.</p>
 */
@Configuration
public class TmdbClientConfig {
     ...
     * base URL, JSON headers, the rate-limit interceptor, the API-key
     * interceptor, and connect/read timeouts.
     *
