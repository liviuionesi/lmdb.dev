...
/**
 * TMDB API response for the paged person-list endpoints — {@code /search/person} and {@code
 * /person/popular}, which share one envelope shape.
 *
...
public record TmdbPersonSearchResponse(
  ...
  /**
   * A person as the list endpoints report them — a lightweight subset of the detail endpoint's
   * profile, which is why upserts from here are stubs.
   *
