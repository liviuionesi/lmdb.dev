 ...
 *
 * <p>Exercises the service's logic in isolation with Mockito: both {@link ActorRepository}
 * (PostgreSQL) and {@link TmdbPersonClient} (TMDB HTTP) are mocked, so no Spring context, database
 * or network is involved — the end-to-end wiring is covered separately by {@code
 * ActorServiceIntegrationTest} (Testcontainers + WireMock).
 *
 * <p>The contract under test is ADR-010's read-through/save-through model: PostgreSQL is consulted
 * first and TMDB is only called on a miss, after which the mapped entity is persisted.
 * Complementing that, list endpoints (search, popular) always call TMDB for ranking but upsert
 * every person they return.
 *
 * <p>Since the TMDB API key is now injected transparently by the {@link
 * org.springframework.web.client.RestClient} interceptor, these tests no longer require {@code
 * ReflectionTestUtils} to set a key field — the mock {@link TmdbPersonClient} is called without any
 * key parameter.
 *
 * @see ActorService
 */
...
class ActorServiceTest {
  ...
  @Mock private TmdbPersonClient tmdbPersonClient;

  @Mock private ActorRepository actorRepository;

  @org.mockito.Spy
  private com.filmpire.actor.mapper.ActorMapper actorMapper =
      org.mapstruct.factory.Mappers.getMapper(com.filmpire.actor.mapper.ActorMapper.class);

  @InjectMocks private ActorService actorService;

  /**
   * A persisted actor must be served straight from PostgreSQL. This is the whole point of
   * save-through: once a person is in our catalog, TMDB is no longer in the request path, so the
   * service stays up (and fast) even when TMDB is slow or rate-limiting us.
   */
  @Test
  ...
  /**
   * A cache miss must fetch from TMDB and persist the mapped entity, so the next request for the
   * same person is served locally. Asserting on the saved entity (not just the returned DTO) is
   * what proves the save-through half of the contract actually happened.
   */
  @Test
  ...
  /**
   * TMDB serves a null biography for people it has no write-up for. The entity normalizes that to
   * an empty string so clients never have to null-check a field that is always present in TMDB's
   * own responses.
   */
  @Test
  ...
  /**
   * Filmography must come back newest-first, and credits TMDB has no release date for must sink to
   * the end rather than sorting as if they were the oldest — an unreleased film is not a 1900s
   * film.
   */
  @Test
  ...
  void getFilmographySortsNewestFirst() {
    ...
    // Then
    assertThat(result)
        .extracting(FilmographyEntryDto::title)
        .containsExactly("Newer", "Older", "Undated");
  }

  /**
   * The native API pages in memory because TMDB's movie_credits has no page parameter. Page 2 of a
   * 3-item list at size 2 must therefore hold exactly the one remaining credit, with the totals
  ...
  /**
   * Paging past the end must yield an empty page, not an exception or an index error — TMDB's own
   * list endpoints behave this way, and the facade contract means clients may probe pages blindly.
   */
  @Test
  ...
  /**
   * Images follow the same read-through/save-through rule as the profile: fetched from TMDB once,
   * then persisted onto the actor. Only the CDN reference is stored — never the bytes
   * (ARCHITECTURE.md §3.8).
   */
  @Test
  ...
  /**
   * Once images are persisted, TMDB must drop out of the request path entirely — the same guarantee
   * the profile itself gets.
   */
  @Test
  ...
  void getImagesServedFromPostgres() {
    ...
    assertThat(images).singleElement().extracting(ActorImageDto::filePath).isEqualTo("/cached.jpg");
    verify(tmdbPersonClient, never()).getPersonImages(anyLong());
    verify(actorRepository, never()).save(any(Actor.class));
  }

  /**
   * TMDB reports no images for obscure people. An empty profiles array must map to an empty list
   * rather than blowing up, and a null array (which TMDB has been known to emit) must be tolerated
   * too.
   */
  @Test
  ...
  /**
   * Search always hits TMDB (we don't reimplement its relevance ranking), but every result must be
   * upserted so the catalog grows from real traffic — that is what makes this a clone rather than a
   * proxy.
   */
  @Test
  ...
  void searchUpsertsResults() {
    ...
    when(tmdbPersonClient.searchPersons("pitt", 1))
        .thenReturn(
            new TmdbPersonSearchResponse(
                1, 1, 2L, List.of(summary(287L, "Brad Pitt"), summary(288L, "Michael Pitt"))));
  ...
  /**
   * A list-endpoint hit carries far less than a detail fetch. Upserting one over an
   * already-detailed row must not wipe the richer fields — losing a biography just because someone
   * searched for that actor would be a silent data-loss bug.
   */
  @Test
  ...
  /**
   * Popular shares search's envelope and upsert path, so it must grow the catalog the same way —
   * this is the cheapest way to seed the database with people users are actually likely to look up.
   */
  @Test
  @DisplayName("getPopular: TMDB ranking passed through, every person upserted")
  void getPopularUpsertsResults() {
    // Given
    when(tmdbPersonClient.getPopularPersons(1))
        .thenReturn(
  ...
  /**
   * TMDB omits the paging fields on some responses. The service must fall back to the requested
   * page and zeroed totals rather than NPE-ing while unboxing them.
   */
  @Test
  ...
  void searchHandlesMissingPagingFields() {
    ...
    when(tmdbPersonClient.searchPersons("pitt", 3))
        .thenReturn(new TmdbPersonSearchResponse(null, null, null, List.of()));
  ...
  private static TmdbPersonResponse tmdbPerson() {
    return new TmdbPersonResponse(
        ...
        "Brad Pitt",
        "An actor.",
        LocalDate.of(1963, Month.DECEMBER, 18),
        "Shawnee, Oklahoma, USA",
        "/profile.jpg",
        50.0,
        List.of("William Bradley Pitt"),
        "Acting",
        2,
        "nm0000093",
        "https://bradpitt.com",
