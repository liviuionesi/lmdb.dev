...
/**
 * Unit tests for {@link IpFilterGlobalFilter}, the gateway's IP allow/deny filter.
 *
 * <p>The downstream {@link GatewayFilterChain} is mocked to complete empty, so a completed chain
 * means "request admitted" and a mutated 403 response means "request blocked". Covers both the
 * request-filtering decisions and the blacklist/whitelist management API that drives them.
 */
...
class IpFilterGlobalFilterTest {
  ...
  /**
   * The default-open case: an IP that is on no blacklist must be admitted, so ordinary traffic
   * flows unless explicitly denied.
   */
  @Test
  ...
  /**
   * A request from a blacklisted IP must be stopped with 403 before reaching any service — the core
   * deny-listing behavior. Asserted on the mutated response status because the filter
  ...
  /**
   * The blacklist must behave as a set that supports add and remove, since operators toggle entries
   * at runtime; a broken remove would strand an IP as permanently blocked.
   */
  @Test
  ...
  /**
   * The whitelist mirrors the blacklist's add/remove semantics — verified independently because the
   * two lists are separate state and a copy-paste bug could wire one to the other.
   */
  @Test
  ...
  /**
   * Whitelist mode must default OFF (fail-open to blacklisting) and toggle cleanly, because
   * flipping it ON changes the filter from deny-list to allow-list semantics — a
   * security-significant switch that must be deliberate, never stuck.
   */
  @Test
  ...
  /**
   * Actuator endpoints must bypass IP filtering even for a blacklisted IP, so health/readiness
   * probes (which may originate from an infrastructure address that happens to be blocked) never
   * get cut off and take the platform down. A null status confirms the filter didn't touch the
   * response.
   */
  @Test
