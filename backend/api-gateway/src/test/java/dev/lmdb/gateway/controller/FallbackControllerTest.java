...
/**
 * Unit tests for {@link FallbackController}, the destination Resilience4j routes to when a
 * downstream service's circuit breaker is open.
 *
 * <p>Each per-service fallback must return a consistent, client-friendly degraded response: HTTP
 * 503 in the shared {@link ApiResponse} envelope with {@code success=false} and a message naming
 * the affected service. These tests assert that contract per service so a failing dependency yields
 * a clear "temporarily unavailable" rather than a raw gateway error or a hang.
 */
@SpringBootTest
@ActiveProfiles("test")
...
class FallbackControllerTest {
  ...
  @Autowired private FallbackController fallbackController;

  /**
   * Movie-service fallback must be a 503 ApiResponse naming "Movie Service", so a tripped
   * movie-service breaker degrades gracefully.
   */
  @Test
  ...
  /**
   * Auth-service fallback must be a 503 ApiResponse naming "Authentication Service" — the message
   * wording differs from the route name, so this pins the human-facing text.
   */
  @Test
