...
import dev.lmdb.gateway.filter.IpFilterGlobalFilter;
...
/**
 * Unit tests for {@link AdminController}, the runtime IP-security management API over {@link
 * IpFilterGlobalFilter}.
 *
 * <p>Wired with a real filter (not a mock) so each test asserts BOTH halves of the contract: the
 * controller returns a well-formed {@link ApiResponse}, AND the operation actually mutated the
 * underlying filter state. That end-to-end check matters because these endpoints let an operator
 * change the gateway's security posture live — a response that claims success without applying the
 * change would be a dangerous silent failure.
 */
...
class AdminControllerTest {
  ...
  /**
   * A fresh gateway must report an empty blacklist — the baseline before any management action, so
   * later add/remove assertions start from a known state.
   */
  @Test
  ...
  void getBlacklist_shouldReturnEmptyBlacklist() {
    ...
    ApiResponse<Set<String>> body =
        Objects.requireNonNull(response.getBody(), "Response body must not be null");
    assertThat(body.isSuccess()).isTrue();
    assertThat(body.getData()).isEmpty();
  }

  /**
   * Adding an IP must both return success AND land in the filter's blacklist — the follow-up filter
   * check is what proves the endpoint has real effect, not just a cosmetic OK response.
   */
  @Test
  ...
  /**
   * Removing a previously-blacklisted IP must clear it from the filter, so an operator can actually
   * un-block an address — verified against the filter, not just the response.
   */
  @Test
  ...
  /**
   * The whitelist must also start empty — baseline for the whitelist management tests, mirroring
   * the blacklist baseline.
   */
  @Test
  ...
  /**
   * Adding to the whitelist must reach the filter's whitelist specifically (not the blacklist) —
   * the separate assertion guards against the two management paths being crossed.
   */
  @Test
  ...
  /**
   * Removing from the whitelist must clear the entry from the filter, the whitelist counterpart to
   * blacklist removal.
   */
  @Test
  ...
  /**
   * Enabling whitelist mode flips the gateway from deny-list to allow-list enforcement — a
   * high-impact switch, so the test confirms the flag actually changed on the filter, not merely
   * that the endpoint answered OK.
   */
  @Test
  ...
  /**
   * Disabling whitelist mode must return the gateway to blacklist enforcement — the reverse switch,
   * verified on the filter flag so the mode can be toggled back safely.
   */
  @Test
  ...
  /**
   * The status endpoint must report the live security posture accurately: with two blacklisted IPs,
   * one whitelisted, and whitelist mode on, the returned sizes and flag must match exactly — this
   * is the dashboard an operator trusts, so wrong counts would mislead security decisions.
   */
  @Test
  ...
  /**
   * The blacklist must behave correctly at set scale: adding three IPs then removing one must leave
   * exactly the other two, proving the underlying set handles multiple entries and selective
   * removal (not just single-entry happy paths).
   */
  @Test
  ...
  void blacklist_shouldManageMultipleIps() {
    ...
    assertThat(ipFilterGlobalFilter.getBlacklist())
        .containsExactlyInAnyOrder("192.168.1.1", "192.168.1.3");
  }

  /**
   * The whitelist counterpart of the multi-IP test: three adds then one removal must leave exactly
   * two, confirming multi-entry management works independently for the whitelist.
   */
  @Test
  ...
  void whitelist_shouldManageMultipleIps() {
    ...
    assertThat(ipFilterGlobalFilter.getWhitelist())
        .containsExactlyInAnyOrder("10.0.0.1", "10.0.0.3");
  }
}
