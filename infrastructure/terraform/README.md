...
```bash
APP_ID=$(az ad app create --display-name lmdb-github-actions --query appId -o tsv)
...
```bash
export TF_STATE_BUCKET=lmdb-tfstate-$RANDOM   # must be globally unique
export TF_STATE_TABLE=lmdb-tfstate-lock
